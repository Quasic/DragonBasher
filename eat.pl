...
if ($slot<24) {
  if (length($item)==2) {
    if (substr($player{'inven'},$slot*10,2) eq $item) {
      if ($item eq "Fa") { $food="Fa"; $health=1; $new="Za00000000"; }
      if ($item eq "Fb") { $food="Fb"; $health=1; $new="Fa".substr($player{'inven'},($slot*10)+2,8); }
      if ($item eq "Fc") { $food="Fc"; $health=1; $new="Fb".substr($player{'inven'},($slot*10)+2,8); }
      if ($item eq "Fd") { $food="Fd"; $health=1; $new="Fc".substr($player{'inven'},($slot*10)+2,8); }
